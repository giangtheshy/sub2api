      ...
      <div class="flex-1">
        ...
        <div v-if="showMethodSelection" class="mb-4">
          ...
          <div class="flex flex-wrap gap-4">
            ...
            <label
              v-if="showImportCredentialsOption"
              class="flex cursor-pointer items-center gap-2"
            >
              <input
                v-model="inputMethod"
                type="radio"
                value="import_credentials"
                class="text-blue-600 focus:ring-blue-500"
              />
              <span class="text-sm text-blue-900 dark:text-blue-200">{{
                t('admin.accounts.oauth.importCredentials')
              }}</span>
            </label>
        ...
        <div v-if="inputMethod === 'import_credentials'" class="space-y-4">
          <div
            class="rounded-lg border border-blue-300 bg-white/80 p-4 dark:border-blue-600 dark:bg-gray-800/80"
          >
            <p class="mb-3 text-sm text-blue-700 dark:text-blue-300">
              {{ t('admin.accounts.oauth.importCredentialsDesc') }}
            </p>
            ...
            <div class="mb-4">
              <div class="mb-2 flex items-center justify-between gap-2">
                ...
                >
                  <Icon name="key" size="sm" class="text-blue-500" />
                  {{ t('admin.accounts.oauth.importCredentialsInputLabel') }}
                </label>
                <button
                  type="button"
                  class="btn btn-secondary btn-sm shrink-0"
                  @click="importCredentialsFileRef?.click()"
                >
                  <Icon name="upload" size="sm" class="mr-1" />
                  {{ t('admin.accounts.oauth.importCredentialsUpload') }}
                </button>
                <input
                  ref="importCredentialsFileRef"
                  type="file"
                  accept=".json,application/json"
                  class="hidden"
                  @change="handleImportCredentialsFileSelected"
                />
              </div>
              <textarea
                v-model="importCredentialsInput"
                rows="8"
                class="input w-full resize-y font-mono text-sm"
                :placeholder="t('admin.accounts.oauth.importCredentialsPlaceholder')"
                spellcheck="false"
              ></textarea>
              <!-- Static example, kept out of i18n so the literal JSON braces
                   are never parsed as vue-i18n interpolation placeholders. -->
              <pre
                class="mt-2 overflow-x-auto rounded bg-gray-100 p-2 text-xs text-gray-500 dark:bg-gray-900 dark:text-gray-400"
              >{{ importCredentialsExample }}</pre>
              <p class="mt-1 text-xs text-blue-600 dark:text-blue-400">
                {{ t('admin.accounts.oauth.importCredentialsHint') }}
              </p>
            </div>
            ...
            <button
              type="button"
              class="btn btn-primary w-full"
              :disabled="loading || !importCredentialsInput.trim()"
            ...
            >
              ...
              {{
                loading
                  ? t('admin.accounts.oauth.authorizing')
                  : t('admin.accounts.oauth.importCredentialsSubmit')
              }}
            </button>
          </div>
        </div>
...
interface Props {
  ...
  /** Whether to show the "import OAuth credentials" option (claudeAiOauth JSON). */
...
const props = withDefaults(defineProps<Props>(), {
  ...
  showImportCredentialsOption: false,
...
const emit = defineEmits<{
  ...
  /** Already-issued OAuth credential file (claudeAiOauth JSON). */
...
const importCredentialsInput = ref('')
const importCredentialsFileRef = ref<HTMLInputElement | null>(null)
// Plain JS string, not an i18n message: it holds literal JSON braces, which
// vue-i18n would otherwise try to parse as interpolation placeholders.
const importCredentialsExample =
  '{"claudeAiOauth":{"accessToken":"sk-ant-oat01-...","refreshToken":"sk-ant-ort01-...","expiresAt":1787183506000,"subscriptionType":"max_20x"}}'
...
const methodOptionCount = computed(() => [
  ...
  props.showImportCredentialsOption,
...
const handleImportCredentialsFileSelected = async (event: Event) => {
  const input = event.target as HTMLInputElement
  const file = input.files?.[0]
  // Reset so picking the same file again still fires a change event.
  input.value = ''
  ...
  if (file.size > MAX_COOKIE_FILE_BYTES) return
  try {
    importCredentialsInput.value = await file.text()
  } catch {
    // Leave the textarea untouched; the operator can paste manually.
  }
}

const handleImportCredentials = () => {
  if (importCredentialsInput.value.trim()) {
    emit('import-oauth-credentials', importCredentialsInput.value.trim())
  }
}
