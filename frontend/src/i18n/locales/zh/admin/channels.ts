export default {
    ...
    riskControl: {
      ...
      blocks: {
        title: '生效中的会话屏蔽',
        hint: '触发上游硬阻断的标识会在本地被拒绝，直到 TTL 到期。手动解除后立即恢复放行。',
        empty: '当前没有被屏蔽的标识。',
        identity: '标识',
        kind: '命中层级',
        signal: '信号',
        ...
        loadFailed: '加载屏蔽列表失败',
        kinds: {
          xUserIdSession: '终端用户 + 会话',
          xUserId: '终端用户（X-User-Id）',
          session: '会话',
          apikeyUser: 'API Key + 用户（整把 Key）',
          unknown: '未标注',
        },
        signals: {
          refusal: '硬拒答（stop_reason）',
          permissionError: '权限拒绝（403）',
          cyberPolicy: '网络安全策略',
        },
      },
